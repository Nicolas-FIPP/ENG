 ...
 */
 ...
 function mMoeda(event) {
   ...
   var v = (event.value).substring(3); // extrai os 3 primeiros caracteres relativos ao 'R$ '
 
   // Faz uma série de substituições nas Expressões Regulares que podem gerar valores monetários
   v = v.replace(/\D/g, ""); // Remove todos os caracteres que não são dígitos
   v = v.replace(/^0+/g, ""); // Remove zeros à esquerda
   
   // Adiciona ponto a cada três dígitos
