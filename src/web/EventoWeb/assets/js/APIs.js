...
function ValidarCampos ()
{
  let DataIni = document.getElementById("DATAINI").value;
  let DataFim = document.getElementById("DATAFIM").value;
  let Agora = new Date(); Agora.setSeconds(0,0);
  ...
  try {
    ...
    if (Array.isArray(dados)) {
      ...
      if (Cep.value != "" && Dados != null)
      {
        ...
        for (let i = 0; i < options.length; i++)
          if (options[i].value == Dados.localidade)
            options[i].selected = true;  
      }
...
async function BuscarPessoas() {
  try {
    ...
    const response = await fetch(`http://localhost:3344/pessoa/`,{
      method: 'GET'
    });
    const dados = await response.json();
...
function GETALL(data) {
  ...
  data.forEach(item => {
      ...
      row.innerHTML = `
          ...
          <td>${item.nome}</td>
          <td>${item.custo}</td>
          <td data-date="${item.dt_ini}">${DiaIni+"/"+MesIni+"/"+AnoIni+"\n"+(HorasIni<10 ? '0'+HorasIni:HorasIni)+":"+(MinutosIni<10 ? '0'+MinutosIni:MinutosIni)}</td>
          <td data-date="${item.dt_fim}">${DiaFim+"/"+MesFim+"/"+AnoFim+"\n"+(HorasFim<10 ? '0'+HorasFim:HorasFim)+":"+(MinutosFim<10 ? '0'+MinutosFim:MinutosFim)}</td>
          <td>${item.cep}</td>
          <td>${item.uf}</td>
          <td>${item.cidade}</td>
          <td>${item.bairro}</td>
...
async function CarregaIdParaUpdate (event)
{
  ...
  for (let i=0; i < option.length ;i++)
    if (option[i].innerText == responsavel)
